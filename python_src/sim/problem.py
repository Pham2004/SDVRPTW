...
class Request:
    def __init__(self, idx: int, x: float, y: float, demand: float, open: float,
                 close: float, service_time: float, time: float,
                 profit: float = 0.0, type: int = 0):
        self.idx = (idx)
        self.x = (x)
        self.y = (y)
        self.demand = (demand)
...
class Problem:
    ...
    def tensor_load(tensor_load: str, instance_num: int = 0, truck_speed: float = 1.0, truck_capacity: float = 1.0, num_trucks: int = 1) -> "Problem":
        ...
        # Keep torch optional for the normal CSV/simulator path.
        import torch
