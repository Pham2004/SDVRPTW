...
- `Simulation`: event-driven simulation that batches requests into time
    ...
    sequencing decisions, and returns `(total_distance, total_profit)`.
...
try:
    ...
    from .ctx import RoutingContext, SequencingContext
except ImportError:  # direct execution with python_src on sys.path
    from gp.GPtree import Program
    from sim.problem import Problem
...
class Simulation:
    def __init__(self, problem : Problem, routing_rule: Program, sequencing_rule: Program):
        ...
        self._counter = 0
        self._has_run = False
        self.dropped_requests: Dict[int, str] = {}
    ...
    def simulate_until(self, time_slot: float, time_max: float) -> Tuple[float, float]:
        if self._has_run:
            raise RuntimeError("Simulation instances are single-use; create a new Simulation")
        if time_slot < 0:
            raise ValueError("time_slot must be non-negative")
        if self.problem.truck_speed <= 0:
            raise ValueError("truck_speed must be positive")
        if self.problem.num_trucks <= 0:
            raise ValueError("num_trucks must be positive")
        self._has_run = True
    ...
    def handle_request(self, request, _cb=None, total_profit_container=None):
        ...
        if vehicle is not None:
            ...
            self.dropped_requests[request.idx] = "no_time_feasible_vehicle"
    ...
    def update_vehicle_queue(self, vehicle: int, _cb, total_profit_container: List[float], total_distance_container: List[float]) -> None:
        ...
        while True:
            ...
            queue = state.queue
            request = queue[idx][0]
            if request.demand > self.problem.truck_capacity:
                queue.pop(idx)
                state.dropped[request.idx] = state.dropped.get(request.idx, 0) + 1
                self.dropped_requests[request.idx] = "demand_exceeds_vehicle_capacity"
                cache.pop(request.idx, None)
                continue
    ...
    def route_vehicle_to(self, vehicle: int, request, _cb, total_distance_container: List[float], total_profit_container: Optional[List[float]] = None) -> None:
        ...
        self._push_event(ev)
        # Preserve every visit even when zero-distance/zero-service visits have
        # the same timestamp. A plain dict assignment used to overwrite one.
        route_time = time - getattr(request, "service_time", 0.0)
        while route_time in state.route:
            route_time = math.nextafter(route_time, math.inf)
        state.route[route_time] = getattr(request, "idx", 0)
    ...
    def routing_rule_route_request(self, problem, time: float, vehicles: List[VehicleState], request) -> Optional[int]:
        ...
        for i in range(len(vehicles)):
            ...
            earliest_departure = max(time, vehicles[i].busy_until)
            if earliest_departure + cost <= request.close:
                # evaluate program
