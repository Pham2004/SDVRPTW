...
from sim.report import print_solution
...
SCENARIO_TRAIN_RATIO = float(os.environ.get("SCENARIO_TRAIN_RATIO", "0.05"))
PRINT_ROUTES         = os.environ.get("PRINT_ROUTES", "1") != "0"
...
def gp(problem_set: ProblemSet):
    ...
    for gen in range(1, NUM_GEN + 1):
        ...
        if gen == NUM_GEN:
            ...
            if PRINT_ROUTES:
                for i, (prob, t_slot) in enumerate(zip(problem_set, time_slots)):
                    s = sim_mod.Simulation(prob, pop[0].routing, pop[0].sequencing)
                    dist, profit = s.simulate_until(t_slot, float("inf"))
                    print_solution(prob, s.get_routes(), dist, profit,
                                   title=f"GP best – scenario {i + 1}")
            for ind in pop:
